...
//        db.execSQL("INSERT INTO " + MEDICINE_MEDICINE_INFO_TABLE_NAME + "(" + MEDICINE_MEDICINE_INFO_COLUMN_MEDICINE_ID + "," + MEDICINE_MEDICINE_INFO_COLUMN_MEDICINE_INFO_ID + ") VALUES (5,5)");
    ...
    public boolean checkIfMedicineEmpty(){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor query = db.rawQuery("SELECT COUNT(*) FROM " + MEDICINE_TABLE_NAME, null);
        query.moveToFirst();
        int rowCount = query.getInt(0);
        if(rowCount <= 0){
            return true;
        } else {
            return false;
    ...
    public boolean checkIfMedicine_infoEmpty(){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor query = db.rawQuery("SELECT COUNT(*) FROM " + MEDICINE_INFO_TABLE_NAME, null);
        query.moveToFirst();
        int rowCount = query.getInt(0);
        if(rowCount <= 0){
            return true;
    ...
    public boolean checkIfMedicine_Medicine_infoEmpty(){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor query = db.rawQuery("SELECT COUNT(*) FROM " + MEDICINE_MEDICINE_INFO_TABLE_NAME, null);
        query.moveToFirst();
        int rowCount = query.getInt(0);
        if(rowCount <= 0){
            return true;
